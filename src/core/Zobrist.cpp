...
uint64_t fodder::getPieceKey(Piece Pc, Square Sq) {
  return PIECE_KEYS[Pc][Sq];
}

uint64_t fodder::getColorKey(Color Col, Square Sq) {
  return COLOR_KEYS[Col][Sq];
}

uint64_t fodder::getEPKey(Square Sq) { return EP_KEYS[Sq]; }

uint64_t fodder::getCastleKey(uint8_t Rights) { return CASTLE_KEYS[Rights]; }

uint64_t fodder::getStmKey() { return STM_KEY; }

std::mt19937_64 randHash(0x123456789);

void fodder::initZobrist() {
